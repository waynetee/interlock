...
TCL
echo ===PROGRAM_START===
timeout 1100 "$A/bin64/FPExpress_bin" SCRIPT:/tmp/prog.tcl LOGFILE:/tmp/prog.log CONSOLE_MODE:brief 2>&1 \
  | grep -aiE "PROGRAM|PASSED|FAILED|Chain programming|Executing action|IDCODE|Error|succeeded" | grep -aviE "qt.qpa|NDDF|cyusb" | tail -25
