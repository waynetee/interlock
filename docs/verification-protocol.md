...
**Challenge-time ingress is committed.** The recomputation interlock feeds the enclosure exactly three things, folding every byte that crosses the boundary through the same traffic-commitment hierarchy as the production interlock (the **challenge-slice commitment**). The verifier later checks that commitment against the packets opened from the network certificates; a mismatch fails the challenge, and a failed challenge is charged as unexplained:
1. the input-packet ciphertext (the verifier checks the committed bytes against `H1_in`);
2. the cryptographic material (key, nonces; checked against `H2`). This is what keeps H2 load-bearing here: the node needs the key to decrypt the prompt and to compute per-position encrypted output units, but the key material enters *after* the response exists — unpinned, the staged blob could smuggle the answers in (e.g., contain the response itself alongside the true key). H2 was fixed in the input packet at request time, before the response existed, so staged material chosen retroactively to fit a covert payload cannot match the commitment — the smuggling challenge fails and is charged;
...
The checks the earlier design ran at the interlock's ingress thus migrate to the verifier: the interlock attests what crossed the boundary; the verifier compares.
...
**Certificate:** the current instantiation **reuses the production certificate builder and format** unchanged: `m = (VERSION, DEVICE, BKT_START, BKT_NUM, NONCE, INWARD, OUTWARD, CHAIN)` with INWARD carrying the challenge-slice commitment (recomputed from the bytes that actually crossed the boundary — none trusted from the prover) and OUTWARD carrying the result `(challenged ID ‖ Û)`, zero-padded; `τ = HMAC_k(m)`. The device consequently keeps the production timeline machinery — buckets, sync packets, certificate tiling — rather than being stateless. Every field is prover-predictable, so the byte-audit and slot-bound cut-and-choose carry over. There is no duration field — a measured-time value has slack the prover cannot predict (the same covert-channel reasoning that removed drop counters); the verifier times the challenge on its own clock.

Both instantiations end at the same interface. The ZKP binds `(H1_in, H2, H1_out)` directly inside the circuit; Option 1 binds the challenge-time bytes via the slice commitment, which the verifier checks against the same triple opened from the network certificates — H2 still pins what may enter the enclosure after the response exists, checked by the verifier instead of the interlock.
