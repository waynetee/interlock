package eth_pkg;
  ...
  typedef logic [ETH_HDR_BYTES-1:0][7:0] eth_hdr_bytes_t;   // [k] = wire byte k
  ...
  function automatic eth_header_t eth_hdr_from_bytes(input eth_hdr_bytes_t b);
    eth_hdr_bytes_t r;
    for (int k = 0; k < ETH_HDR_BYTES; k++) r[ETH_HDR_BYTES-1-k] = b[k];
  ...
  function automatic eth_hdr_bytes_t eth_hdr_to_bytes(input eth_header_t h);
    ...
    eth_hdr_bytes_t b;
    for (int k = 0; k < ETH_HDR_BYTES; k++) b[k] = r[ETH_HDR_BYTES-1-k];
    return b;
  endfunction
